...
검색은 주소(시군구)와 **장소명(`name`) 모두** 부분 일치한다(구현 완료). **범위 밖**: 리뷰, 코스 생성. **마커 클러스터링은 방식 미정(보류)** — 결정 후 본 스펙을 갱신해 이번 작업 단위 안에서 진행한다.
...
- **적용**: 목록(`GET /places`)은 옵셔널 JWT(비로그인 거리순), 검색(`GET /places/search`)은 **JWT 필수**(비로그인 401). 인증된 요청이면 저장된 `member.filter_tags`로 (matched, 거리, id) 정렬, 목록 비로그인은 거리순만. **쿼리 파라미터 필터는 없다** — 저장된 값만 사용한다.
