...
- 기존 **409 `MEMBER_409_1`(`WITHDRAWAL_LOCKED`)은 더 이상 던지지 않는다.** 에러 코드는 남겨두되 사용처가 없어진다.

**왜 에러가 아니라 200인가** — 전역 예외 핸들러가 `ApiResponse<Void>`를 돌려줘 **에러 응답에는 데이터를 실을 수 없다**. 날짜를 주려면 (가) 예외에 payload를 실을 수 있게 전역 구조를 바꾸거나 (나) 정상 응답의 상태 분기로 옮겨야 한다. 이미 `WITHDRAWAL_PENDING`을 200으로 내리고 있어 (나)가 기존 구조와 일관되고, 한 건 때문에 전역 예외 구조를 흔들지 않아도 된다.
