...
| 2026-08-14 | **Implemented** | 구현 완료 — 10개 항목 전부. 즉시 탈퇴는 배포 환경이 하나뿐이라 프로파일 게이트를 걷어내고 그대로 배포한다. V22(`review.content` NULL 허용)·V23(`verified_level` 교체), 재가입 대기 안내는 [ADR 0013](../adr/0013-withdrawal-locked-as-status.md)으로 기록. 구현 중 추가된 것: 매핑 없는 요청의 500 → 404·405 수정, 토큰 재발급 응답에도 `isOnboarded` |
...
| 9 | 내부 테스트용 즉시 탈퇴 API | 신규 API | 003 |
| 10 | 연습 목록 `visitedAt` → `lastActivityAt` | 필드 의미 변경 | 011 |

1~6·8·10은 프론트 요청, 7은 서버 발의, 9는 테스트 편의를 위한 API다(앱 debug 빌드 전용).
...
9. **즉시 탈퇴 API(내부 테스트용)** — 유예기간 없이 회원을 물리 삭제해, 같은 소셜 계정으로 바로 재가입·온보딩을 다시 테스트할 수 있게 한다. 앱 debug 빌드에서만 연결한다.
...
| DELETE | /api/v1/members/me/hard | 즉시 탈퇴(내부 테스트용) | JWT | **신규** |
...
**게이트 없이 배포한다.** 프로파일이나 설정 값으로 가리는 방식을 검토했으나, **배포 환경이 하나뿐이라 운영 서버가 곧 테스트 서버다**(develop·main 모두 같은 서버로 배포되고 컨테이너는 `SPRING_PROFILES_ACTIVE=prod`). 가리면 로컬에만 존재해 정작 앱이 붙는 서버에서 쓸 수 없다.

**노출은 막히지 않는다는 점을 감수한다.** 앱은 debug 빌드에만 이 API를 연결하지만, 화면에 버튼이 없을 뿐 토큰이 있으면 누구나 호출할 수 있다. **본인 계정만 지워지는 것이 유일한 제약**이라 타인 데이터는 안전하고, 최악의 경우는 사용자가 자기 계정을 유예기간 없이 삭제하는 것이다. 필요 없어지면 컨트롤러·서비스 파일을 지워 제거한다.
...
5. **즉시 탈퇴는 물리 삭제, 게이트 없이 배포** — 익명화 배치를 즉시 실행하는 방식은 결과가 비슷하면서 구현만 복잡해 택하지 않았다. 프로파일·설정 게이트는 배포 환경이 하나뿐이라 로컬 전용이 되어버려 목적을 잃는다. 노출 제한은 앱이 debug 빌드에서만 연결하는 것으로 갈음한다.
