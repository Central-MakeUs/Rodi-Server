...
| 각 지점명(`name`) | 앱(주소/장소명 일부) | 그대로 저장. 요청 `name`이 없으면 **출발지 지점명이 코스명이 된다**(아래) |
...
1. **코스 등록**: 출발지 1 · 경유지 0~3 · 도착지 1, 선택 코스명, 주행거리, 연습유형 1~3개, 한줄소개(10~30자), 주의사항(선택, 100자)을 받아 코스를 만든다. `name`이 있으면 그 값을 저장하고, 없으면 출발지(`START`) 지점명을 코스명으로 쓴다. 상태는 **`PENDING`(승인 대기)** 로 시작한다.
...
| `waypoints[].name` | **START만 Y** | 최대 255자. 요청 `name`이 없으면 **`START`의 값이 코스명이 된다**(아래) |
