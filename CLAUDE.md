...
| 테스트 | JUnit 5, Testcontainers, Mockito, AssertJ |
| 품질/검사 | Spotless, SonarCloud, JaCoCo, CodeRabbit |
| CI/CD·배포 | GitHub Actions, Docker, GHCR, AWS Lightsail, Caddy |
| 의존성 | Dependabot |
...
- **컨트롤러 분리 기준**: **자체 오퍼레이션 묶음을 가진 하위 리소스는 전용 컨트롤러**로 분리하고, **단발 엔드포인트는 소유(가장 가까운) 컨트롤러**에 둔다. 같은 URL 스코프여도 이 기준을 우선한다. 예: `recent-searches`(GET·DELETE·DELETE/{id}) → `RecentSearchController`; `filter-tags`(PUT 1개) → `MemberController`. URL 일관성이 아니라 **기준의 일관 적용**이 목표다.
