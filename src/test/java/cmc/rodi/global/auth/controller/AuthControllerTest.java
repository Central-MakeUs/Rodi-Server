...
class AuthControllerTest {
    ...
    void 소셜_로그인_성공() throws Exception {
        when(authService.login(eq(SocialProvider.KAKAO), eq("kakao-token")))
                .thenReturn(
                        SocialLoginResponse.success(
                                new Tokens("access-jwt", "refresh-raw"),
                                true,
                                false,
                                false,
                                "차근차근 토끼"));
        ...
        mockMvc.perform(
                ...
                .andExpect(jsonPath("$.data.isCourseTutorialCompleted").value(false))
    ...
    void 재발급_성공() throws Exception {
        when(authService.reissue("refresh-raw"))
                .thenReturn(new TokenResponse("new-access", "new-refresh", true, true));
        ...
        mockMvc.perform(
                ...
                .andExpect(jsonPath("$.data.isCourseTutorialCompleted").value(true))
